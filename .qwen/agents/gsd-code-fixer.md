...
**Project instructions:** Read `./QWEN.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during fixes.
...
Read `./QWEN.md` and check for `.qwen/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO respect QWEN.md project conventions** during fixes. If project requires specific patterns (e.g., no `any` types, specific error handling), apply them.
...
- [ ] Project conventions from QWEN.md respected during fixes
