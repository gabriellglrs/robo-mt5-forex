 ...
 *   - generate-claude-profile: Developer Profile section in QWEN.md
 *   - generate-claude-md: full QWEN.md with managed sections
 */
...
const CLAUDE_MD_FALLBACKS = {
  ...
  skills: 'No project skills found. Add skills to any of: `.qwen/skills/`, `.agents/skills/`, `.cursor/skills/`, or `.github/skills/` with a `SKILL.md` index file.',
};

// Directories where project skills may live (checked in order)
const SKILL_SEARCH_DIRS = ['.qwen/skills', '.agents/skills', '.cursor/skills', '.github/skills'];
 ...
 * (name + description) for each. Returns a table summary for QWEN.md so
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
    targetPath = path.join(os.homedir(), '.claude', 'QWEN.md');
  } else if (options.output) {
    targetPath = path.isAbsolute(options.output) ? options.output : path.join(cwd, options.output);
  } else {
    // Read claude_md_path from config, default to ./QWEN.md
    let configClaudeMdPath = './QWEN.md';
    try {
