...
```bash
INIT=$(node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
AGENT_SKILLS_EXECUTOR=$(node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-executor 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
AGENT_SKILLS_VERIFIER=$(node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-verifier 2>/dev/null)
```
...
- ./QWEN.md (if exists — project-specific guidelines)
...
- ./QWEN.md (if exists — follow project-specific guidelines)
...
- .planning/STATE.md (Project state)
- ./QWEN.md (Project instructions, if exists)
...
**Known Qwen Code bug (classifyHandoffIfNeeded):** If executor reports "failed" with error `classifyHandoffIfNeeded is not defined`, this is a Qwen Code runtime bug — not a real failure. Check if summary file exists and git log shows commits. If so, treat as successful.
...
node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
