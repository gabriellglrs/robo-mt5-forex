...
```bash
PHASE_INFO=$(node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "C:/DEV/robo-mt5-v2/.qwen/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
